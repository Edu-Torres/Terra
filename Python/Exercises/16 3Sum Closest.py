def threeSumClosest(nums, target):
    ...
    for i in range(0, len(nums) - 2):
        for j in range(i + 1, len(nums) - 1):
            ...
            while sta < end:
                mid = (sta + end) // 2
                if abs(new_target - nums[mid]) < closeness:
                    closeness = abs(new_target - nums[mid])
                    ans = nums[mid] + nums[i] + nums[j]
                    if ans == -2804:
                        print(closeness, ans)
